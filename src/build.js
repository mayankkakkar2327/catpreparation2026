...
function hero(eyebrow, title, answer, ctas = []) {
  return `<section class="hero"><div class="container hero-grid"><div><p class="eyebrow">${esc(eyebrow)}</p><h1>${esc(title)}</h1>${answer ? `<p class="answer">${esc(answer)}</p>` : ""}<div class="actions">${ctas.map(c => `<a class="btn ${c.primary ? "primary" : ""}" href="${c.href}">${esc(c.label)} <span aria-hidden="true">→</span></a>`).join("")}</div></div><div class="hero-object" aria-hidden="true"><div class="orbit orbit-one"></div><div class="orbit orbit-two"></div><div class="hero-card"><span>CAT 2026</span><strong>Research before you enrol.</strong><small>Coaching · Exams · Colleges</small></div></div></div></section>`;
}
function card(c) {
